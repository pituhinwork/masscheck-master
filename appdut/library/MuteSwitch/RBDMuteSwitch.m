...
//
//  Created by Vu Thu on 18/04/2012.
//  Copyright (c)  All rights reserved by R&D Solutions Limited.
//

#import "RBDMuteSwitch.h"

#define MUTEBUG
#if defined(MUTEBUG) || defined(DEBUG_ALL)
#   define RBDMuteSwitchLog(BachUng, ...) NSLog((@"%s[Line %d] " BachUng), __FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
#   define RBDMuteSwitchLog(...)
#endif
...
- (void)playbackComplete {
    if ([(id)self.delegate respondsToSelector:@selector(isMuted:)])
    {
        ...
        if (soundDuration < 0.07)//0.01
        {
//             RBDMuteSwitchLog(@"[RBDMUTESWITCH] soundDuration :                                    %f",soundDuration);
...
//    }
    ...
    AudioServicesAddSystemSoundCompletion (soundFileObject,NULL,NULL,  soundCompletionCallback, (__bridge void*) self);
